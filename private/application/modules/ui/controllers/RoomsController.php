...
class Ui_RoomsController extends IMDT_Controller_Abstract {
    ...
    public function init() {
        ...
        $arrBbbOptions['lock_allow_moderator_locking'] = array('name'=>'lock_allow_moderator_locking','label'=> $this->_helper->translate('column-meeting_room-lock_allow_moderator_locking'),'type'=>'boolean');
        $arrBbbOptions['lock_disable_mic_for_locked_users'] = array('name'=>'lock_disable_mic_for_locked_users','label'=> $this->_helper->translate('column-meeting_room-lock_disable_mic_for_locked_users'),'type'=>'boolean');
        $arrBbbOptions['lock_disable_cam_for_locked_users'] = array('name'=>'lock_disable_cam_for_locked_users','label'=> $this->_helper->translate('column-meeting_room-lock_disable_cam_for_locked_users'),'type'=>'boolean');
    ...
    public function exportAction() {
        ...
        $headers['columns-leach'] .= 'meeting_mute_on_start,meeting_lock_on_start,lock_allow_moderator_locking,lock_disable_mic_for_locked_users,lock_disable_cam_for_locked_users,lock_disable_public_chat_for_locked_users,lock_disable_private_chat_for_locked_users';
    ...
    public function tableContentAction() {
        ...
        if (IMDT_Util_Acl::getInstance()->isAllowed('rooms', 'duplicate')){
            ...
            $actions .= '<a title="' . $this->_helper->translate('Duplicate') . '" data-toggle="tooltip" class="btn btn-mini" disabled="disabled"><i class="icon-file"></i></a>';
                ...
                if ($curr['_removable'] == '1') {
                    $actions .= '<a title="' . $this->_helper->translate('Delete') . '" data-toggle="tooltip" class="btn btn-mini btn-delete" href="" data-original-title="Excluir" onclick="deleteRow(\'#records\',\'row_' . $curr[$this->pkey] . '\',\'/ui/rooms/delete/id/' . $curr[$this->pkey] . '\'); return false;"><i class="icon-trash"></i></a>';
                } else {
    ...
    public function formContentAction() {
        ...
        try {
            ...
            if ($id == 'new') {
                ...
                $arrForm['lock_allow_moderator_locking'] = $arrBbbConfigs['lockAllowModeratorLocking'];
                ...
                $arrForm['lock_allow_moderator_locking'] = $row['lock_allow_moderator_locking'];
    ...
    public function formPostAction() {
        ...
        try {
            ...
            $data['lock_allow_moderator_locking'] = $this->_request->getPost('lock_allow_moderator_locking', '0');
