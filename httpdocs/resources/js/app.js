...
$(document).ready(function() {
    $("a.btn.add").live("click", function() {
		...
		if(modal.find('.row-filter').size() > 1) {
	...
	if($(this).hasClass('rows-selectable')) {
		...
		if (e.target.nodeName.toLowerCase() == 'a' || e.target.nodeName.toLowerCase() == 'i') return;
        		
		var checkbox = $(this).find('input.cboxSelectRow');
		if(checkbox.size() == 0 || $(checkbox).is(':disabled')) return;
...
applyFilter = function (tableSelector,contextSelector) {
    ...
    objectFilters.musthave = $(parent).find('input[name="musthave"]:checked').val();
    objectFilters.q = [];
	
    $(parent).find('#frmMainFilter div.row-filter').each(function() {
		...
		if(value != null && typeof value == 'object') value = value.join();
    ...
    $(parent).find('#frmAdvFilter div.row-filter').each(function() {
		...
		if(objColumn.v != null && typeof objColumn.v == 'object') objColumn.v = objColumn.v.join();
    ...
    if(table.attr('uri-nofilters') != undefined) {
...
$(document).ready(function(){
	...
	if(controlKeys.join(",").match(new RegExp(event.which)) || numberKeys.join(",").match(new RegExp(event.which))) return;
	else event.preventDefault();
    }).addClass('.int_binded');
	
    $("input.numeric").not('.numeric_binded').keypress(function(event) {
	if(controlKeys.join(",").match(new RegExp(event.which)) || numberKeys.join(",").match(new RegExp(event.which)) || "44,46,45".match(new RegExp(event.which))) return;
	else event.preventDefault();
    }).addClass('numeric_binded');
	
    $("input.ipaddress").not('.ipaddress_binded').keypress(function(event) {
	if(controlKeys.join(",").match(new RegExp(event.which)) || numberKeys.join(",").match(new RegExp(event.which)) || "46".match(new RegExp(event.which))) return;
	else event.preventDefault();
    ...
    $('form.ajaxsubmit').submit(function() {
		...
		$.ajax({
		    ...
		    success     : function(a) {
			if(a.success == 1) {
			    if(a.redirect) _redirect(a.redirect);
			    if(a.msg) _alert({
				type : 'success', 
    ...
    $('div.span9').delegate("a[open-modal-form]", "click", function(e) {
		...
		if($(this).attr('form-data')) {
			...
			if(jsonData[prop] == null || jsonData[prop].length == 0) continue;
		...
		$(form).not('.submitbinded').submit(function() {
			...
			success     : function(a) {
				...
				if(typeof a.msg != 'undefined') {
				    modal.find('.modal-footer .success-buttons .alert-success').html(a.msg);
				}
...
_redirect = function(url) {
    if(!url || url == 'undefined' || url == '') url = window.location.protocol + '//' + window.location.host;
    window.location = url;
};
